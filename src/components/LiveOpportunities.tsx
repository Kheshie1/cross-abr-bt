    ...
    <Card className="bg-card border-border border-l-2 border-l-accent">
      <CardHeader className="pb-3">
        <CardTitle className="text-sm font-medium text-accent flex items-center gap-2">
          ...
          LIVE PROFIT — ENDING SOON
          {isLoading && <Loader2 className="ml-2 inline h-3 w-3 animate-spin" />}
        </CardTitle>
        <p className="text-xs text-muted-foreground">
          Markets resolving within 48h with 93%+ confidence — buy now, profit on resolution
        </p>
      </CardHeader>
      <CardContent className="space-y-2 max-h-[450px] overflow-y-auto">
        {live.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-8">
            {isLoading ? "Scanning live markets..." : "No live opportunities right now"}
          </p>
        ) : (
          live.map((m: any) => (
            <div
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-2">
                  <Badge className="bg-profit/15 text-profit border-profit/30 text-xs">
